...
type Action="approve"|"decline"|"confirm"|"copy";
...
export default function OwnerCalendar(){
  ...
  async function review(id:number,action:Action){
    ...
    else if(action==="copy")setMessage("Email copy sent.");
  ...
  return <>
    <section className="requestPanel">
      ...
      <div className="requestList">{requests.length===0?<p>No requests yet.</p>:requests.map(r=><article className="requestCard" key={r.id}>
        ...
        {r.status==="approved"&&<div className="reviewActions"><button disabled={working===r.id} onClick={()=>review(r.id,"confirm")}>{working===r.id?"Working…":"Mark deposit received"}</button><button className="secondary" disabled={working===r.id} onClick={()=>review(r.id,"copy")}>Email me a copy</button></div>}
        {(r.status==="declined"||r.status==="confirmed")&&<div className="reviewActions"><button className="secondary" disabled={working===r.id} onClick={()=>review(r.id,"copy")}>{working===r.id?"Sending…":"Email me a copy"}</button></div>}
      </article>)}</div>
    </section>
