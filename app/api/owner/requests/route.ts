...
export async function GET(){if(!await ownerOr401())return Response.json({error:"Unauthorized"},{status:401});const rows=await getDb().select().from(bookingRequests).orderBy(desc(bookingRequests.createdAt));return Response.json({requests:rows});}
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(body.action==="decline"){
      await db.update(bookingRequests).set({status:"declined"}).where(eq(bookingRequests.id,booking.id));
    ...
    await Promise.allSettled([
      sendMail({to:booking.email,subject:"Your stay at The Vues is approved",html:`<h2>Your request is approved</h2><p>Hi ${escapeHtml(booking.name)}, your stay from ${escapeHtml(booking.arrival)} through ${escapeHtml(booking.departure)} has been approved.</p><p><strong>Total: ${money(quote.totalCents)}</strong>, including 6% Michigan lodging tax${booking.boatRental?` and ${money(quote.boatRentalCents)} for pontoon / jet-ski rental`:""}.</p><p>To hold the dates, please send a <strong>${money(depositCents)} deposit</strong> using <a href="https://venmo.com/u/KlingerLake68109">Venmo @KlingerLake68109</a>, or reply for Zelle instructions. Include memo <strong>${memo}</strong>. Your dates are confirmed after the owner verifies receipt.</p><p><strong>Cancellation policy:</strong> Email bockal@gmail.com at least seven full days before check-in for a full refund. Requests received fewer than seven days before check-in are not eligible for a full refund.</p>`}),
